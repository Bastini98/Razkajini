...
import { ShoppingCart, CheckCircle, Truck, RotateCcw, Award, Star } from "lucide-react";
...
const AddToCartAudio: React.FC<AddToCartAudioProps> = ({ product, className }) => {
  ...
  const miniStars = Math.max(0, Math.min(5, Math.round(Number(mini?.stars ?? 5))));
  const miniQuote = String(mini?.quote ?? "").replace(/^["“]|["”]$/g, "");

  return (
    <div className={`sticky top-24 bg-white rounded-2xl border border-sand/30 p-6 shadow-lg space-y-6 ${className || ""}`}>
      {/* Цена */}
      ...
      {/* Бележка: дигитална доставка */}
      <p className="text-sm text-ink/90 text-center bg-sand-light/40 rounded-xl p-3">
        <CheckCircle className="inline h-4 w-4 mr-1 text-teal" />
        <strong>След покупка</strong> ще получите имейл с <strong>линк за изтегляне</strong> на приказката (MP3).
      </p>
      ...
      {mini && (
        <div className="bg-gradient-to-br from-sand-light/30 to-white rounded-2xl p-6 border border-sand/20">
          ...
          {miniQuote && <blockquote className="text-gray-700 italic mb-2">“{miniQuote}”</blockquote>}
          <div className="text-sm">
            <div className="font-semibold text-ink">{String(mini.author ?? "Анонимен")}</div>
            <div className="text-xs text-gray-500">{String(mini.label ?? "Слушател")}</div>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="text-center p-4 bg-white rounded-xl border border-sand/20">
          ...
          <p className="text-xs text-gray-600">Моментален достъп</p>
        </div>
        <div className="text-center p-4 bg-white rounded-xl border border-sand/20">
          <RotateCcw className="h-6 w-6 text-teal mx-auto mb-2" />
          <p className="text-xs text-gray-600">Слушай офлайн</p>
        </div>
        <div className="text-center p-4 bg-white rounded-xl border border-sand/20">
          <Award className="h-6 w-6 text-teал mx-auto mb-2" />
          <p className="text-xs text-gray-600">Проф. запис</p>
        </div>
      </div>
    </div>
  );
