// src/components/InsideBooksTestimonials.tsx
...
const clampStars = (v?: number) => Math.max(0, Math.min(5, Math.round(v ?? 5)));
const cleanQuote = (s?: string) => (s ? s.replace(/^["“]|["”]$/g, "") : "");

const InsideBooksTestimonials: React.FC<Props> = ({
  testimonials,
  initialCount = 3, // по 3
  step = 3,         // показва още 3
  title = "Какво казват хората, които направиха подарък от нас",
  subtitle = "Истински отзиви от семейства, които запазиха спомените си завинаги.",
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 max-w-6xl mx-auto px-4">
        <AnimatePresence initial={false}>
          {shown.map((t, idx) => {
            ...
            const label = t.label || "Клиент";
            return (
              <motion.blockquote
                key={`${author}-${idx}-${quote.slice(0, 10)}`}
                layout
...
export default InsideBooksTestimonials;
